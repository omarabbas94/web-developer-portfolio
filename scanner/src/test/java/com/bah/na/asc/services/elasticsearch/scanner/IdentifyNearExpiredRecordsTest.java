...
public class IdentifyNearExpiredRecordsTest{
	...
	public void setUp() throws Exception{
		...
		try{
			connection = new ESConnection(esIp, Integer.parseInt(esTransportPort), esClusterName);
		...
		/*
		 * URL jsonFileUrl =
		 * getClass().getResource("/data/uid-email-expirationdate.json"); byte[]
		 * buffer = Files.readAllBytes(Paths.get(jsonFileUrl.toURI())); // Bulk
		 * Ingest Test Data try (TransportClient transportClient =
		 * connection.getTransportClient()) { BulkRequestBuilder bulkBuilder =
		 * transportClient.prepareBulk(); bulkBuilder.add(buffer, 0,
		 * buffer.length); bulkBuilder.get(); // Wait a few seconds
		 * Thread.sleep(4000);
		 */
	}
	...
	@Ignore
	@Test
	public void testIngestAndIdentify() throws Exception{
